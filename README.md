...
- ERD 및 DB를 담당하셨던 분이 화면 구현 담당자에게 넘겨주셨을 때, 화면 구현 담당자가 table생성을 새로 insert 문으로 해야하는 줄 알고 헤매다가, csv로 간단하게 table 생성이 가능하다는 것을 알고 문제를 해결하였다.
...
# 🖊 회고
 - 원유형 
> 화면 설계를 하면서 figma 사용 방법에 대해서 배울 수 있었고 streamlit 의 기능 또한 익힐 수 있었습니다. 설계 후 팀원들과 의견이 달라서 여러 번의 수정을 했는데 프로젝트 시작시에 소통이 잘 안 되었다고 생각이 들어 소통의 중요성에 대해 다시금 깨닫게 되었습니다. git으로 협업을 처음 해보았기에 git 사용법에 익숙치 않아 문제들이 발생했고 이를 해결하는 방법을 터득했습니다. 다음 프로젝트 시에는 더 원활하게 협업을 할 수 있을 것입니다. 처음에 파트 별로 일정을 나눠서 시간이 여유가 있다고 생각을 하였는데 작업이 계속 미뤄져서 일정이 바빠졌습니다. 프로젝트 시 항상 여유 시간을 가지고 작업을 해야 deadline에 맞출 수 있다는 것을 알게 되었습니다.
